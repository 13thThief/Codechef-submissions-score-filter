...
( function() {
    ...
    $( document ).ready( function() {
        ...
        $( ".topbox-inner td:last" )
            .before( "<td style='border-bottom:0px;' class='l-float'> <div class='l-float'> <input type='number' max='100' id='scorefilter' name='scorefilter' placeholder='Score' class='field-style-text'/> </div> </td>" );
        ...
        if( ind != -1 ) {
            score += "pts]";
            ct = 0;
            $( "#primary-content .dataTable tbody>tr" )
                .each( function () {
            ...
            for( i = 1; i <= numpages; i++ ) {
                ...
                $.ajax({
                    ...
                    success: function( data ) {
                        var lct = 0;
                        $( data ).find( "#primary-content .dataTable tbody>tr" )
                            .each( function () {
                                var ro = $( this ).find( "td span" ).text();
                                var ln = ro.length;
                                if( ro.slice( ln - 6 ) == score ) {
